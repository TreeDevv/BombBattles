...
type TargetingOptions = {
	...
	cameraTweenSeconds: number?,
	focusResponsiveness: number?,
	targetRayDistance: number?,
...
local OverheadPlacementTargeting = {}
local Targeter = {}
Targeter.__index = Targeter

local LocalPlayer = Players.LocalPlayer
local DEFAULT_CAMERA_TWEEN_SECONDS = 0.4
local DEFAULT_FOCUS_RESPONSIVENESS = 14
local DEFAULT_TARGET_RAY_DISTANCE = 10000
local TARGET_RAY_MODE_CAMERA_DOWN = "CameraDown"
...
local function newState()
	return {
		...
		abilityId = "",
		definition = nil,
		savedCamera = nil :: SavedCameraState?,
		markerFolder = nil,
		radiusPart = nil,
		centerPart = nil,
		ui = nil,
...
function OverheadPlacementTargeting.new(options: TargetingOptions)
	return setmetatable({
		options = options,
		state = newState(),
	}, Targeter)
...
function Targeter:IsActive(): boolean
	return self.state.active == true
end

function Targeter:GetSessionId(): number
	return self.state.sessionId
end

function Targeter:getRoundState()
	local callback = self.options.getRoundState
	return if callback then callback() else nil
end

function Targeter:getTargetRoot(): Model?
...
function Targeter:resolvePlacementForward(rootPart: BasePart): Vector3
	local map = self:getTargetRoot()
...
function Targeter:getControls(): Controls?
...
function Targeter:disableMovementControls()
	local state = self.state
	if state.controlsDisabled then
		return
	end

	local controls = state.controls or self:getControls()
...
function Targeter:restoreMovementControls()
	local state = self.state
...
function Targeter:disconnectAll()
...
function Targeter:updateKeyboardInput()
	local input = Vector2.zero
	for keyCode, held in pairs(self.state.heldMoveKeys) do
		if held then
			input += KEY_DIRECTIONS[keyCode] or Vector2.zero
		end
	end

	self.state.keyboardInput = if input.Magnitude > 1 then input.Unit else input
end

function Targeter:clearMoveInput()
	local state = self.state
	table.clear(state.heldMoveKeys)
...
function Targeter:getMarkerFolder(): Folder
	local state = self.state
	...
	folder.Name = self.options.previewFolderName
	folder.Parent = workspace
	state.markerFolder = folder
	return folder
end

function Targeter:createMarkerPart(name: string, size: Vector3, transparency: number): BasePart
	...
	part.Size = size
	part.Parent = self:getMarkerFolder()
	return part
end

function Targeter:ensureMarker(definition: AbilityDefinition)
	local state = self.state
	local prefix = self.options.markerPrefix or self.options.abilityId
	local radius = getDefinitionNumber(definition, self.options.radiusKey or "strikeRadius", self.options.radiusFallback or 22)
	if not state.radiusPart then
		state.radiusPart = self:createMarkerPart(prefix .. "Radius", Vector3.new(0.08, radius * 2, radius * 2), 0.7)
	end
	if not state.centerPart then
		state.centerPart = self:createMarkerPart(prefix .. "Center", Vector3.new(0.16, 2.5, 2.5), 0.25)
	end
end

function Targeter:setMarker(position: Vector3?, valid: boolean)
	local state = self.state
...
function Targeter:destroyPreview()
	local state = self.state
	if state.markerFolder and state.markerFolder.Parent then
...
function Targeter:getMoveInput(): Vector2
	local state = self.state
...
function Targeter:getPanDirection(input: Vector2): Vector3
	if input.Magnitude <= 0.05 then
		return Vector3.zero
	end

	local forward = flattenDirection(self.state.placementForward)
	if forward.Magnitude <= 0.05 then
...
function Targeter:raycastPlacement(camera: Camera): (Vector3?, Vector3?, Vector3?)
	...
	local maxDistance = self.options.targetRayDistance or DEFAULT_TARGET_RAY_DISTANCE
	local hit = workspace:Raycast(rayOrigin, rayDirection.Unit * maxDistance, makeRaycastParams())
	if not hit then
		return nil, rayOrigin, rayDirection
	end
...
function Targeter:updatePlacementFromMove(definition: AbilityDefinition, camera: Camera, dt: number): (Vector3?, Vector3?, Vector3?, Vector3, boolean)
	...
	local panDirection = self:getPanDirection(self:getMoveInput())
...
function Targeter:getCameraGoal(definition: AbilityDefinition, focus: Vector3): (CFrame, number)
...
function Targeter:getBlendAlpha(): number
	local value = self.state.tweenValue
	return if value then math.clamp(value.Value, 0, 1) else 1
end

function Targeter:restoreCamera()
	local camera = workspace.CurrentCamera
	local saved = self.state.savedCamera
...
function Targeter:Cancel(snap: boolean?)
	local state = self.state
	...
	if snap == true or not (camera and saved and state.definition) then
		self:finishCleanup()
		return
	end

	local sessionId = state.sessionId
	local restoreAlpha = self:getBlendAlpha()
	...
	state.tween = TweenService:Create(
		value,
		TweenInfo.new(self.options.cameraTweenSeconds or DEFAULT_CAMERA_TWEEN_SECONDS, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
		{ Value = 0 }
	)
...
function Targeter:startTweenIn()
	self:cancelTween()
	local value = Instance.new("NumberValue")
	value.Value = 0
	self.state.tweenValue = value
	self.state.tween = TweenService:Create(
		value,
		TweenInfo.new(self.options.cameraTweenSeconds or DEFAULT_CAMERA_TWEEN_SECONDS, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
		{ Value = 1 }
	)
	self.state.tween.Completed:Once(function()
		if self.state.tweenValue == value then
			self.state.tween = nil
		end
	end)
	self.state.tween:Play()
end

function Targeter:stepTargeting(dt: number)
	local state = self.state
	local camera = workspace.CurrentCamera
	...
	if not state.restoreTweening then
		...
		state.valid = valid
		self:setMarker(targetPosition, valid)
		...
		state.currentFocus = if state.currentFocus
			then state.currentFocus:Lerp(desiredFocus, 1 - math.exp(-(self.options.focusResponsiveness or DEFAULT_FOCUS_RESPONSIVENESS) * dt))
			else desiredFocus
	end

	local focus = state.currentFocus or saved.cframe.Position
	local overheadCFrame, overheadFOV = self:getCameraGoal(definition, focus)
	local alpha = self:getBlendAlpha()
...
function Targeter:ConfirmTarget()
	local state = self.state
	if
		not (
			state.active
...
function Targeter:RequestCancel()
	local state = self.state
	if not state.active then
		return
	end
...
function Targeter:handleTargetingAction(_actionName: string, inputState: Enum.UserInputState, inputObject: InputObject)
	local state = self.state
	local keyDirection = KEY_DIRECTIONS[inputObject.KeyCode]
	if keyDirection then
		if inputState == Enum.UserInputState.Begin then
			state.heldMoveKeys[inputObject.KeyCode] = true
		elseif inputState == Enum.UserInputState.End or inputState == Enum.UserInputState.Cancel then
			state.heldMoveKeys[inputObject.KeyCode] = nil
		end
...
function Targeter:updateTouchPadVisual()
	local knob = self.state.touchKnob
	if not knob then
		return
	end

	knob.Position = UDim2.fromScale(0.5, 0.5)
		+ UDim2.fromOffset(self.state.touchInput.X * 42, -self.state.touchInput.Y * 42)
end

function Targeter:setTouchInputFromPosition(screenPosition: Vector3)
	local state = self.state
...
function Targeter:clearTouchInput(inputObject: InputObject?)
	local state = self.state
...
function Targeter:createTouchMovePad(parent: Instance)
	...
	table.insert(state.connections, pad.InputBegan:Connect(function(input: InputObject)
		...
		self:setTouchInputFromPosition(input.Position)
	end))
	...
	table.insert(state.connections, UserInputService.InputEnded:Connect(function(input: InputObject)
		if input.UserInputType == Enum.UserInputType.Touch then
			self:clearTouchInput(input)
		end
	end))
end

function Targeter:createUi()
	local gui = Instance.new("ScreenGui")
	gui.Name = self.options.uiName or (self.options.abilityId .. "TargetingGui")
	...
	table.insert(self.state.connections, confirm.Activated:Connect(function()
		self:ConfirmTarget()
	end))
	table.insert(self.state.connections, cancel.Activated:Connect(function()
		self:RequestCancel()
	end))
	self:createTouchMovePad(gui)

	gui.Parent = LocalPlayer:WaitForChild("PlayerGui")
	self.state.ui = gui
end

function Targeter:bindCleanupSignals()
	table.insert(self.state.connections, LocalPlayer.CharacterRemoving:Connect(function()
		self:Cancel(true)
	end))
	...
	if humanoid then
		table.insert(self.state.connections, humanoid.Died:Connect(function()
			self:Cancel(true)
		end))
	end
end

function Targeter:Begin(context: ClientEffectContext)
	local payload = context.payload
	if typeof(payload) ~= "table" or payload.player ~= LocalPlayer or payload.abilityId ~= self.options.abilityId then
		return
	end
	...
	local definition = AbilityConfig.GetDefinition(self.options.abilityId)
	local camera = workspace.CurrentCamera
	local rootPart = getRootPart()
	...
	state.abilityId = self.options.abilityId
	...
	state.placementForward = self:resolvePlacementForward(rootPart)
	...
	self:ensureMarker(definition)
	self:createUi()
	self:bindCleanupSignals()

	ContextActionService:UnbindAction(self.options.actionName)
	ContextActionService:BindActionAtPriority(
		self.options.actionName,
		function(actionName: string, inputState: Enum.UserInputState, inputObject: InputObject)
			return self:handleTargetingAction(actionName, inputState, inputObject)
		end,
		false,
		self.options.actionPriority or (Enum.ContextActionPriority.High.Value + 80),
	...
	RunService:BindToRenderStep(
		self.options.renderStepName,
		self.options.renderPriority or (Enum.RenderPriority.Camera.Value + 6),
		function(dt: number)
			self:stepTargeting(dt)
		end
	)
	self:startTweenIn()
end

function Targeter:OnActivateRequested(context: ClientActivateRequestedContext): boolean
...
return OverheadPlacementTargeting
