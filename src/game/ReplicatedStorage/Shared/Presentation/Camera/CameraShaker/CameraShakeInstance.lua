...
function CameraShakeInstance.new(magnitude, roughness, fadeInTime, fadeOutTime)
	...
	local self = setmetatable({
		...
		currentFadeTime = if fadeInTime > 0 then 0 else 1,
...
function CameraShakeInstance:UpdateShake(dt)
	local currentTick = self.tick
	local currentFadeTime = self.currentFadeTime

	local offset = V3(
		NOISE(currentTick, 0) * 0.5,
		NOISE(0, currentTick) * 0.5,
		NOISE(currentTick, currentTick) * 0.5
	)
	...
	if self.sustain then
		self.tick = currentTick + (dt * self.Roughness * self.roughMod)
	else
		self.tick = currentTick + (dt * self.Roughness * self.roughMod * currentFadeTime)
	end
...
function CameraShakeInstance:SetScaleRoughness(value)
	self.roughMod = value
end
...
function CameraShakeInstance:SetScaleMagnitude(value)
	self.magnMod = value
end
