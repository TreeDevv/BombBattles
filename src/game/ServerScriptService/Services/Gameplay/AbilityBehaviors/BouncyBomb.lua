...
local BouncyBomb = {} :: AbilityTypes.ServerBehavior
...
local function createProjectileId(player: Player): string
	projectileSerial += 1
	return ("BouncyBomb_%d_%d_%04d"):format(
		player.UserId,
...
function BouncyBomb.CanActivate(context: ServerActivateContext): boolean
	return getCharacterRoot(context.player) ~= nil and getBombProjectileService() ~= nil
end

function BouncyBomb.OnActivate(context: ServerActivateContext): AbilityActivationResult
	...
	local bouncyBombsFired = if typeof(state) == "table" and typeof(state.bouncyBombsFired) == "number"
		then state.bouncyBombsFired
		else 0

	return {
		state = {
			bouncyBombsFired = bouncyBombsFired + 1,
			lastActivatedAt = context.now,
		},
		effect = {
			name = "BouncyBombFired",
			payload = {
				projectileId = projectileId,
			},
		},
