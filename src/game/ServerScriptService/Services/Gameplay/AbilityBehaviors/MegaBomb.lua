...
local MegaBomb = {} :: AbilityTypes.ServerBehavior
...
local function createProjectileId(player: Player): string
	projectileSerial += 1
	return ("MegaBomb_%d_%d_%04d"):format(
		player.UserId,
...
function MegaBomb.CanActivate(context: ServerActivateContext): boolean
	return getCharacterRoot(context.player) ~= nil and getBombProjectileService() ~= nil
end

function MegaBomb.OnActivate(context: ServerActivateContext): AbilityActivationResult
	...
	local scale = math.max(getDefinitionNumber(context.definition, "megaScale", 2), 0.05)
	...
	local launched = projectileService:Launch({
		...
		modifier = {
			physics = {
				radius = BombConfig.SweepRadius * scale,
	...
	local megaBombsThrown = if typeof(state) == "table" and typeof(state.megaBombsThrown) == "number"
		then state.megaBombsThrown
		else 0

	return {
		state = {
			megaBombsThrown = megaBombsThrown + 1,
