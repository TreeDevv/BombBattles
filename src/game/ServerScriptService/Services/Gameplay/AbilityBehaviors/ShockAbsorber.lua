...
local ShockAbsorber = {} :: AbilityTypes.ServerBehavior

local DEFAULT_ACTIVE_UNTIL_ATTR = "ShockAbsorber_ActiveUntil"
...
local function getMultiplier(definition, key: string, fallback: number): number
	return math.clamp(tonumber(definition and definition[key]) or fallback, 0, 1)
end
...
function ShockAbsorber.CanActivate(context: ServerActivateContext): boolean
	local _character, _humanoid, rootPart = getCharacterParts(context.player)
	return rootPart ~= nil
end

function ShockAbsorber.OnActivate(context: ServerActivateContext): AbilityActivationResult
...
function ShockAbsorber.OnBeforePlayerBombDamage(context: ServerHookContext): AbilityHookResult
	...
	return {
		...
		damageMultiplier = getMultiplier(context.definition, "damageMultiplier", 0.5),
		knockbackMultiplier = getMultiplier(context.definition, "knockbackMultiplier", 0.5),
	}
end

function ShockAbsorber.OnPlayerRemoving(player: Player)
	clearActiveRecord(player, nil, nil, nil)
end
