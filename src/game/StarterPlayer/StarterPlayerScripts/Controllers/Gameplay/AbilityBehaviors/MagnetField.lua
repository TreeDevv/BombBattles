...
local MagnetField = {} :: AbilityTypes.ClientBehavior

local PREVIEW_FOLDER_NAME = "MagnetFieldPreview"
local VISUAL_FOLDER_NAME = "MagnetFieldVisuals"
local HIGHLIGHT_NAME = "MagnetFieldInsideHighlight"
local RENDER_STEP_NAME = "BombBattlesMagnetFieldPreview"
local RENDER_PRIORITY = Enum.RenderPriority.Camera.Value + 3
local COMMIT_ACTION_NAME = "BombBattlesMagnetFieldCommit"
...
local function styleGhost(ghost: Instance, definition: AbilityDefinition)
	styleParts(ghost, definition.previewValidColor or Color3.fromRGB(255, 221, 76), definition.previewTransparency or 0.58)
	setHighlightStyle(ghost, definition.previewValidColor or Color3.fromRGB(255, 221, 76), 0.9, 0.2)
	disableVfx(ghost)
end

local function setGhostColor(ghost: Instance, definition: AbilityDefinition, valid: boolean)
	local color = if valid
		then definition.previewValidColor or Color3.fromRGB(255, 221, 76)
...
local function getEmitModule()
	...
	if not (moduleScript and moduleScript:IsA("ModuleScript")) then
		if not warnedMissingEmitModule then
			warn("[MagnetField] Missing ReplicatedStorage.Packages.EmitModule")
	...
	if not ok then
		if not warnedMissingEmitModule then
			warn("[MagnetField] Failed to require EmitModule: " .. tostring(result))
			warnedMissingEmitModule = true
		end
		return nil
	end
...
local function ensureEmitModuleInitialized(module): boolean
	...
	if type(initFn) == "function" then
		...
		if not ok then
			warn("[MagnetField] Failed to initialize EmitModule: " .. tostring(err))
			return false
		end
	end
...
local function emitCenter(root: Instance)
	...
	if not ok then
		warn("[MagnetField] Failed to emit center VFX: " .. tostring(err))
	end
end
...
local function addOrUpdateHighlight(player: Player, character: Model, definition: AbilityDefinition)
	...
	local color = definition.visualColor or Color3.fromRGB(255, 221, 76)
...
local function updateHighlights()
	local definition = AbilityConfig.GetDefinition("MagnetField")
...
local function createPulseShell(definition: AbilityDefinition, cframe: CFrame, finalSize: Vector3)
	...
	shell.Name = "MagnetFieldPulse"
	pivotTo(shell, cframe)
	disableVfx(shell)
	setHighlightStyle(shell, definition.visualColor or Color3.fromRGB(255, 221, 76), 1, 1)
...
local function playPlacedVisual(definition: AbilityDefinition, fieldId: string, cframe: CFrame, radius: number, activeEndsAt: number)
	...
	clone.Name = "MagnetField_" .. fieldId
...
function MagnetField.OnActivateRequested(context: ClientActivateRequestedContext): boolean
...
function MagnetField.OnEffect(context: ClientEffectContext)
	if context.payload.abilityId ~= "MagnetField" then
		return
	end
	...
	if context.effectName == "MagnetFieldPlaced" then
	...
	elseif context.effectName == "MagnetFieldExpired" then
		fadeAndDestroyVisual(fieldId, definition)
	end
end
